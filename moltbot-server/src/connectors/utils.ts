...
export async function getExistingChunks(
  ...
  try {
    const response = await fetch(
      `${config.supabaseUrl}/rest/v1/moltbot_knowledge_chunks?user_id=eq.${encodeURIComponent(userId)}&source_id=eq.${encodeURIComponent(sourceId)}&select=id,external_id,content_hash`,
      {
        headers: {
...
export async function upsertChunk(
  ...
  try {
    ...
    const url = isNew
      ? `${config.supabaseUrl}/rest/v1/moltbot_knowledge_chunks`
      : `${config.supabaseUrl}/rest/v1/moltbot_knowledge_chunks?id=eq.${encodeURIComponent(existing!.id)}`;
...
export async function deleteChunk(chunkId: string): Promise<Result<void>> {
  try {
    const response = await fetch(
      `${config.supabaseUrl}/rest/v1/moltbot_knowledge_chunks?id=eq.${encodeURIComponent(chunkId)}`,
      {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${config.supabaseServiceKey}`,
