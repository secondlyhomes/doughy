...
On iOS 26+, navigation chrome (tab bars, headers) automatically adopts the Liquid Glass appearance. For custom control:

### NativeTabs (iOS Tab Bar)

Use `NativeTabs` from `expo-router/unstable-native-tabs` for a native `UITabBarController` with automatic glass. Fork by platform — NativeTabs is iOS-only:
...
export default function TabLayout() {
  ...
  return (
    <NativeTabs
      ...
      tintColor={DynamicColorIOS({ light: '#4CAF50', dark: '#66BB6A' })}
...
Use `useHeaderHeight()` from `@react-navigation/elements` to offset content below transparent headers.

For the full guide including troubleshooting, see [Liquid Glass](LIQUID-GLASS.md).
...
- [ ] iOS 26 Liquid Glass configured for tab bar and headers
