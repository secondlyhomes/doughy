// src/features/settings/services/seeders/common/images.ts
// Property images for seeding
...
export function getRandomPropertyImage(): string {
  const index = Math.floor(Math.random() * RENTAL_PROPERTY_IMAGES.length);
  return RENTAL_PROPERTY_IMAGES[index];
}
