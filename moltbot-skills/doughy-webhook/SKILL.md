...
## Purpose
Main entry point for MoltBot to process incoming rental platform emails.
Orchestrates all Doughy Edge Functions to handle the complete flow from
email receipt to AI response generation.

## How MoltBot Uses This

When MoltBot receives an email notification via Gmail Pub/Sub:
...
4. **Return result** to MoltBot for logging/monitoring
...
│                                                            │
│  2. moltbot-bridge → UPSERT_CONTACT                        │
│     └─ Create or update contact in database                │
│                                                            │
│  3. moltbot-bridge → GET_PROPERTY                          │
│     └─ Match property from address/name hint               │
│                                                            │
│  4. moltbot-bridge → CREATE_CONVERSATION                   │
...
```typescript
import handleIncomingEmail from './moltbot-skills/doughy-webhook/handler';

// When MoltBot receives an email
...
Deploy the webhook as an HTTP endpoint that MoltBot can call:

```typescript
// In your MoltBot server
...
MoltBot should set up a Gmail watch on the user's inbox:

```typescript
// 1. User authorizes Gmail access via OAuth
// 2. MoltBot creates a Pub/Sub subscription
// 3. When email arrives, Pub/Sub sends notification
// 4. MoltBot fetches the email content
// 5. MoltBot calls this webhook handler
```
